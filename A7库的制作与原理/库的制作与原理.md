...
一个可执行程序，没有被加载到内存，它是有地址的。

对于可执行程序，通过起始地址与偏移量进行编址。这种地址被称为**逻辑地址**。**如今，所有可执行程序就是一个`seg`，所有`seg`、函数、变量编址都是从0开始。这种编址模式是平坦模式。**

==**磁盘上的可执行程序、代码和数据编址其实就是虚拟地址的统一编址。**==

**进程`mm_struct`、`vm_area_struct`在进程刚刚被创建时，初始值是从`ELF`各个`segment`中来的，每个`segment`都有自己的起始地址与长度，用于初始化结构体中`[start,end]`等范围数据。**

![image-20260820172038043](./assets/image-20260820172038043.png)

CPU怎么拿到起始地址呢？

文件`ELF`中有一个`ELF Header`，这里面有一个`Entry Point address`存储了整个程序起始地址，加载到内存后，直接给`CPU`的`EIP`寄存器。
...
在`C/C++`程序中，当程序开始运行时，首先不会跳转到`main`函数，实际上，程序段入口是`_start`，这是一个C运行时库（通常是`glibc`）或链接器提供的特殊函数。

`_start`会加载程序所需的动态库。链接器（如`ld-linux.so`）负责在程序运行时加载动态库。Linux会通过环境变量（如`LD_LIBRARY_PATH`）和配置文件（如`/etc/ld.so.conf`及其子配置文件）来指定动态库的搜索路径。

### 动态库加载

动态链接实际上将链接的整个过程推迟到了程序加载的时候。

动态库也是`ELF`，我们也理解为起始地址（0）+偏移量。

![image-20260820173103014](./assets/image-20260820173103014.png)

![image-20260820184749256](./assets/image-20260820184749256.png)

- 也就是说，我们的程序运行之前，先把所有库加载并映射，所有库的起始虚拟地址都应提前知道。
- 然后对我们加载到内存的程序的库函数调用进行地址修改，在内存中二次完成地址设置（加载地址重定位）
- 但是代码区怎么修改呢？

动态链接采取的做法是在`.data`（可执行程序或库自己）中专门预留一块区域用来存放函数的跳转地址，它也被叫做**全局偏移表`GOT`**，表中每一项都是本运行模块要引用的一个全局变量或函数的地址。

![image-20260820185821663](./assets/image-20260820185821663.png)

`GOT`表是可读可写的，每个进程都会维护一张`GOT`表，进程间不共享`GOT`。

利用`GOT`表这种方式动态链接就被叫做`PIC`与地址无关码。`PIC`=相对编址+`GOT`。**GOT 是一个表，表项内容是“符号在运行时的绝对虚拟地址”。代码通过访问 GOT 表项来间接获得这个绝对地址。call 的时候就是call .got地址+相对.got偏移量，然后跳转到函数的绝对地址。**

库间也有依赖关系，也有`GOT`表。

![image-20260820190730869](./assets/image-20260820190730869.png)
